...
exports.acePostWriteDomLineHTML = function (hook, context) {
  console.log('ep_IMAGE_UPLOAD');
  var img = $(context.node).find('img');
  if (img) {
    console.log('img', img);
    var src = img.attr('src');
    console.log(src);
    if (src) {
      var doUpload = imageUpload.srcCheck(src);
      if (doUpload) {
        var path = imageUpload.uploadFromURL(src);

        imageUpload.uploadFromURL(src).on('complete', function (data) {
  ...
  imageUpload.initS3();
    ...
    context.ace.callWithAce(function(ace) {
      ...
      $(doc).on("drop", function(e){
        e = e.originalEvent;
        var htmlItem = e.dataTransfer.getData('text/html');
        if (htmlItem) {
          var src = $(htmlItem).attr('src');
          console.log(htmlItem);
          console.log($(htmlItem).find('img'))
          console.log('SRC', src);
          imageUpload.uploadFromURL(src).on('complete', function (data) {
            var path = data.request.params.Key;            
            var pathItems = path.split('/');
...
exports.aceEditorCSS = function(hook_name, cb) {
    return ["/ep_copy_paste_images/static/css/ace.css"];
} // inner pad CSS
